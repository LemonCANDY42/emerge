/**
 * Retry helper for the Anthropic provider.
 *
 * Retry strategy:
 ...
 *   - `error.statusCode`  — Anthropic SDK sets this to the HTTP status code on
 *                           APIError subclasses.
 *   - `error.status`      — OpenAI SDK convention; checked for interoperability.
...
export function isRetriable(err: unknown): boolean {
  ...
  // OpenAI SDK convention (in case of interop)

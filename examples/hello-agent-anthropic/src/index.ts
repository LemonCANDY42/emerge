/**
 * hello-agent-anthropic — runs the hello-agent task against a real Anthropic model.
 *
...
async function main() {
  ...
  if (!apiKey) {
    console.log(
      "[skipped: ANTHROPIC_API_KEY not set] — run with:\n  ANTHROPIC_API_KEY=sk-ant-... node examples/hello-agent-anthropic/dist/index.js",
    );
    process.exit(0);
  }

  console.log("=== hello-agent-anthropic ===\n");
  ...
  const sessionId = `hello-anthropic-${Date.now()}` as SessionId;
  ...
  const agentId = "hello-agent-anthropic" as AgentId;
  ...
  console.log("\n=== hello-agent-anthropic complete ===");
}
